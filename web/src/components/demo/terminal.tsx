...
export function Terminal({
  ...
  const [typed, setTyped] = useState(activeLine.length);
  useEffect(() => {
    if (!animate) {
      setTyped(activeLine.length);
      return;
    }
    setTyped(0);
    ...
    const id = setInterval(() => {
      i += 1;
      setTyped(i);
      if (i >= activeLine.length) clearInterval(id);
    }, 16);
  ...
  const cmdDone = typed >= activeLine.length;
    ...
    <div className="flex h-full flex-col overflow-hidden rounded-card border border-night-line bg-night">
      ...
      <div ref={scroller} className="flex-1 space-y-1.5 overflow-y-auto px-4 py-3 font-mono text-[12px] leading-relaxed">
        ...
        {revealed.map((entry, i) => {
          ...
          const shown = isActive ? line.slice(0, typed) : line;
